...
> **`update-rates` cron은 매일 17:00(KST) 딱 1번만 실행됩니다** (`vercel.json`, Vercel Hobby 플랜은 cron을 하루 1회로 제한하므로 재시도 불가). 그 시점에 ECOS에서 확인되는 지표가 하나라도 있으면 그 데이터로 AI 요약 생성과 이메일 다이제스트 발송을 트리거합니다(`daily_summary`에 그날 행 생성). 아직 안 올라온 지표는 그냥 스킵되고, 다음날 다시 시도됩니다. 같은 날짜에 수동으로 다시 호출해도(`daily_summary`에 이미 행이 있으면) 중복 발송되지 않습니다.
