...
describe('buildRatesWorkbook', () => {
  it('produces one row per date with one column per instrument, most recent date first', () => {
    const rows = [
    ...
    expect(data[0]).toMatchObject({ 날짜: '2026-07-28', '국고채 3년': 2.9 })
    expect(data[1]).toMatchObject({ 날짜: '2026-07-27', '국고채 3년': 2.85, 'CD금리 91일': 3.5 })
  })
